...
public class CheckersGUI extends JFrame implements MouseListener,
	...
	public static String languageSelect = "pt";
	
	public ResourceBundle config = new TradutionLang(languageSelect).getBundle(languageSelect);
	...
	private Action newGame, pause, quit, playerSetup, gameOptions, escolherLang,
			help, changeLog, license, about;
	...
	public CheckersGUI(CheckersGameManager gameManager) {
		...
		stateList.setToolTipText(config.getString("Alist")
				+ " " + config.getString("btp"));
		...
		TitledBorder border = BorderFactory.createTitledBorder(BORDER, config.getString("Turns"));
		...
		player2Label.setToolTipText(config.getString("Player2Name"));
		player2Label.setHorizontalAlignment(SwingConstants.CENTER);
		player2Label.setForeground(PLAYER2_COLOR);
		border = BorderFactory.createTitledBorder(BORDER, config.getString("P2") + " "
		...
		player1Label.setToolTipText(config.getString("Player1Name"));
		player1Label.setHorizontalAlignment(SwingConstants.CENTER);
		player1Label.setForeground(PLAYER1_COLOR);
		border = BorderFactory.createTitledBorder(BORDER, config.getString("P1") + " "
		...
		moveCount.setToolTipText(config.getString("Thecurrent"));
		...
		plyTime
				.setToolTipText(config.getString("Showsturn"));
		...
		gameTime
				.setToolTipText(config.getString("Thetotalgame"));
		...
		SUB_PAUSED_TEXT = config.getString("Press") + " '"
				+ getKeyStrokeText((KeyStroke) pause
						.getValue(Action.ACCELERATOR_KEY)) + "' " + config.getString("toUnpause");
	...
	public void gameEnded(CheckersGameEvent ce) {
		if (gameManager.getGameOutcome() == CheckersGameManager.PLAYER1_WINS) {
			plyTime.setText(gameManager.getPlayer1().getName() + " " + config.getString("WINS"));
			plyTime.setForeground(PLAYER1_COLOR);
		} else if (gameManager.getGameOutcome() == CheckersGameManager.PLAYER2_WINS) {
			plyTime.setText(gameManager.getPlayer2().getName() + " " + config.getString("WINS"));
			plyTime.setForeground(PLAYER2_COLOR);
		} else if (gameManager.getGameOutcome() == CheckersGameManager.DRAW) {
			plyTime.setText(config.getString("TheGame"));
	...
	private void initActions() {
		...
		pause = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (gameManager.isPaused()) {
					...
					putValue(Action.NAME, config.getString("Pause"));
				} else if (gameManager.hasGameRunning()) {
					gameManager.setPaused(true);
					putValue(Action.NAME, config.getString("Unpause"));
				}
				repaint();
			}
		};
		...
		escolherLang = new AbstractAction() {
			@Override
			public void actionPerformed(ActionEvent e) {
				...
				ChooseLanguage dialog = new ChooseLanguage(CheckersGUI.this);
				
				if (dialog.isAccepted()) {
					
					System.out.print(dialog.textSelect);
					
					System.out.print("\nLinguagem\n");
					System.out.print(languageSelect);
				
					if(dialog.textSelect == "PT") {
						setLanguageSelect("pt");
						System.out.print("Estou no PT");
					}else if(dialog.textSelect == "EN") {
						setLanguageSelect("en");
						System.out.print("Estou no EN");
					}
		...
		help = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				new TextDialog(CheckersGUI.this, getTitle() + " " + config.getString("Help"),
						HELP_TEXT, true);
		...
		changeLog = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				new TextDialog(CheckersGUI.this, getTitle() + " " + config.getString("ChangeLog"),
		...
		license = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				new TextDialog(CheckersGUI.this, config.getString("GNU"),
						LICENSE_TEXT, false);
		...
		about = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				new TextDialog(CheckersGUI.this, getTitle() + " " + config.getString("About"),
						ABOUT_TEXT, true);
		...
		newGame.putValue(Action.NAME, config.getString("NewGame"));
		pause.putValue(Action.NAME, config.getString("Pause"));
		quit.putValue(Action.NAME, config.getString("Quit"));
		playerSetup.putValue(Action.NAME, config.getString("PlayerSetup"));
		gameOptions.putValue(Action.NAME, config.getString("GameOptions"));
		escolherLang.putValue(Action.NAME, config.getString("escolherLang"));
		help.putValue(Action.NAME, config.getString("Help"));
		changeLog.putValue(Action.NAME, config.getString("ChangeLog"));
		license.putValue(Action.NAME, config.getString("GNU"));
		about.putValue(Action.NAME, config.getString("About"));
		...
		gameOptions.putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(
				KeyEvent.VK_O, 0));
		escolherLang.putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(
				KeyEvent.VK_F4, 0));
	...
	private void initMenu() {
		JMenu file = new JMenu(config.getString("File"));
		file.add(new JMenuItem(newGame));
		file.add(new JMenuItem(pause));
		file.add(new JMenuItem(quit));

		JMenu settings = new JMenu(config.getString("Settings"));
		settings.add(new JMenuItem(playerSetup));
		settings.add(new JMenuItem(gameOptions));

		JMenu toolsM = new JMenu(config.getString("Tools"));
		toolsM.add(new JMenuItem(escolherLang));

		JMenu helpM = new JMenu(config.getString("Help"));
	...
	public void plyTimeChanged(CheckersGameEvent ce) {
		StringBuffer strBuffer = new StringBuffer();
		strBuffer.append(config.getString("TurnTimeLeft") + " ");
		...
		gameTime.setText(config.getString("GameTime") + " "
	...
	private void update() {
		...
		StringBuffer strBuffer = new StringBuffer(config.getString("Mov") + " ");
	...
	public void updatePlayerLabels() {
		...
		((TitledBorder) player1Label.getBorder()).setTitle(config.getString("Player1") + " "
				+ player1.getName());
		((TitledBorder) player2Label.getBorder()).setTitle(config.getString("Player2") + " "
				+ player2.getName());
	}
	...
	public void updatePlayerScores() {
		StringBuffer strBuffer = new StringBuffer(config.getString("Chec") + " ");
		strBuffer.append(selectedState.board
				.getCount(CheckersBoard.PLAYER1_CHECKER));
		strBuffer.append("  " + config.getString("Kings") + " ");
		strBuffer.append(selectedState.board
				.getCount(CheckersBoard.PLAYER1_KING));
		player1Label.setText(strBuffer.toString());

		strBuffer = new StringBuffer(config.getString("Chec") + " ");
		strBuffer.append(selectedState.board
	...
	try {
		currentJar = new File(CheckersGUI.class.getProtectionDomain().getCodeSource().getLocation().toURI());
	  ...
	  try {
		builder.start();
		config = new TradutionLang(languageSelect).getBundle(languageSelect);
		System.out.println("\nEstou no restart\n");
	} catch (IOException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
