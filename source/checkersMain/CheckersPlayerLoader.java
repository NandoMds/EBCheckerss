...
public final class CheckersPlayerLoader {
	...
	private static class JavaClassFileFilter implements FileFilter {
		public boolean accept(File pathname) {
			System.out.println(config.getString("Found") + " " + pathname.getName());
	...
	private static boolean addPlayer(String classPath) {
		try {
			...
			if (CheckersPlayerInterface.class.isAssignableFrom(c)) {
				...
				System.out.println(config.getString("Loadeddd") + ": " + classPath);
				return true;
			}
	...
	public static void loadCheckersPlayers() {
		...
		try {
			System.out.println("\n"+config.getString("CheckersPlayerLoader"));
			...
			try {
				...
				JOptionPane.showMessageDialog(null, e2.getMessage(),
						config.getString("CheckersPlayerLoader"), JOptionPane.ERROR_MESSAGE);
			}
		}

		quickSort();
		System.out.println(config.getString("FinishedSearching")+"\n");
	}
	...
	private static void loadPlayersHelper(File directory) {
		System.out.println(config.getString("FinishedSearching")+" " + directory.toURI().getPath());
