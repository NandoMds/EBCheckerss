...
public class CheckerKnight implements CheckersPlayerInterface {
		...
		protected float dynamic_minimax(int depth, int maxdepth, boolean max) {
			if (depth <= maxdepth) {
				...
				for (int x = 0; x < nextmoves; x++) {
					...
					index++;
					if (Timeleft < 1000) {
						System.out.println(config.getString("Timeout"));
						break;
					}
				}
				...
				if (!children.isEmpty()) {
					for (x = 0; x < children.size(); x++) {
						...
						if (prune_remainder) {
							...
							System.out.println(" "+ config.getString("nodespruned"));
							break;
						}
					}
					if (parent != null) {
	...
	public int choosePlyIndex(CheckersPlayerEvent cpe) {
		...
		for (int id = 1; id < maxdepth; id++) {
			...
			if (Timeleft < 1000) {
				System.out.println(config.getString("Atmaxdepth")+" " + id);
				break;
			} else {
				best_index_yet = tree.root.preferred_child_index;
			}
		}
		System.out.println(config.getString("BestPath")+" " + res);/*
												 * for(BoardNode x =
												 * (BoardNode)tree
												 * .root.best_child
												 * ();x!=null;x=(
	...
	public String getDescription() {
		return config.getString("ACheckersAIhatusesaDepth")
				+ " " + config.getString("Searchwithamini-maxalgorithmandAlpha-BetaPruningggg")
				+ "\n\nAuthor: Matt" + "\nVersion: 1.00 - 10 July 2008";
	}

	@Override
	public String getName() {
		return config.getString("CheckerKnight");
	}
