...
public class ChinookJr implements CheckersPlayerInterface {
	...
	public int choosePlyIndex(CheckersPlayerEvent cpe) {
		...
		debugPrint(config.getString("Analyzingtopnodes"));
		if (cpe.board.getNumSuccessors() > 1) {
			float bestScore = minimax(cpe.board.getSuccessor(0),
					MINIMAX_SEARCH_DEPTH);
			debugPrint("    " + config.getString("Thescoreofply0is") + " " + bestScore);
			System.gc();
			...
			for (int i = 1; i < cpe.board.getNumSuccessors(); i++) {
				...
				debugPrint("    "+ config.getString("Thescoreofply") + " " + i + " " + config.getString("is") +" " + currentScore);
		...
		// scoreCache.clear();
		debugPrint("HEUR_GEN_COUNT "+ config.getString("is") + " " + HEUR_GEN_COUNT);
		HEUR_GEN_COUNT = 0;
		return bestOption;
	}
	...
	public String getDescription() {
		return config.getString("AcheckersAIthattraversesthcheckersgametree")
				+ " " + config.getString("insearchoftheoptimummove.Itsearchestoan")
				+ " " + config.getString("arbitrary,human-setdepth")
				+ " " + config.getString("usingtheminimaxsearchalgorithmanda")
				+ " " + config.getString("heuristicbasedongamepiecepositionsand")
				+ " " + config.getString("quantity.pruning")
				+ " " + config.getString("andminimaxvalue-caching.")
