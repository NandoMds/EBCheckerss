...
public class HAL1000 extends HAL900 {
	...
	public static synchronized boolean loadNeuralNetwork(
		...
		try {
			...
			isr.close();
			fis.close();
			System.out.println(config.getString("FileLoaded") + " - " + file.toURI().getPath());
			return true;
		} catch (Exception e) {
			System.out.println(config.getString("ERRORLoadingFile") + " - "
					+ file.toURI().getPath());
			return false;
		}
	}
	...
	public static synchronized boolean saveNeuralNetwork(
		...
		try {
			...
			pStream.flush();
			pStream.close();
			System.out.println(config.getString("FileSaved") + " - " + file.toURI().getPath());
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			System.out
					.println(config.getString("ERRORSavingFile") + " - " + file.toURI().getPath());
			return false;
		}
	}
	...
	public static void training1(CheckersNN neuralNetwork,
		...
		System.out.println(config.getString("Mean-SquaresError") + ": " + meanSquaresError);
	}
	...
	public static void training2(CheckersNN neuralNetwork,
		...
		System.out.println(config.getString("ExpectedValue") + " " + expectedValue);
		float meanSquaresError = 0;
		int i = moves.size() - 1;
		ListIterator<CheckersBoard> iter = moves.listIterator(moves.size());
		while (iter.hasPrevious()) {
	...
	public static void training3(CheckersNN neuralNetwork,
		...
		System.out.println(config.getString("ExpectedValue1") +" " + expectedValue1);
		...
		System.out.println(config.getString("Mean-SquaresError1") + " " + meanSquaresError1
				+ config.getString("Mean-SquaresError2") + " " + meanSquaresError2);
	}
	...
	public static void training4(CheckersNN neuralNetwork,
		...
		System.out.println(config.getString("Valueee") + " " + value);
		System.out.println(config.getString("ExpecteddValuee") + " " + expectedValue);
	...
	public void gameEnded(CheckersPlayerEvent cpe) {
		super.gameEnded(cpe);
		System.out.println(getName() + " - " + config.getString("ApplyBackPropagation"));
	...
	public String getDescription() {
		return config.getString("AnadvancedlearningCheckersAIthatusesss")
				+ " " + config.getString("searchwithmini-maxalgorithmandalpha-betaprunin")
				+ " " + config.getString("Italsousesauniqueheuristicnwitha")
				+ " " + config.getString("neuralsearchdepth")
				+ "\n\nAuthor: Amos Yuen and Louis Wang\nVersion: " + VERSION;
	}
}
